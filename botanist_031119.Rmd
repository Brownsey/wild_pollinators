...
kmeans_pre_4 <- tibble(cluster = kmeans(data_2012 %>% select(eiqB11F.pre, eiqB11I.pre), 4)$cluster)

data_clusters <- tibble(kmeans_pre_4, data_2012)
data_clusters %>%
  ggplot() +
  geom_point(aes(x = data_2012$orchard, y = data_2012$eiqB11I.pre,colour = factor(kmeans_pre_4$cluster))) + 
  geom_point(aes(x = data_2012$orchard, y = data_2012$eiqB11F.pre,colour = factor(kmeans_pre_4$cluster))) +
  theme_bw()
...
k_clusters %>%
  ggplot() + 
  geom_point(aes(by_day$day_1$bloom.index, y = by_day$day_2$bloom.index, colour = factor(kmeans$kmeans_pre_4$cluster))) +
  theme_bw()
```
