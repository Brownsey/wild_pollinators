...
Clustering code
```{r}
#Clusters of whole data combined - 7 - breaks sometimes with maximum bit weird but maybe another justification method as to why 4 clusters?
#Euclidean clusters.
fviz_nbclust(NbClust(whole_cluster, distance = "euclidean", 
                                min.nc = 2, max.nc = 8, method = "ward.D2", index = "all"))

#when using distance = "maximum,some of the methods break so index = "all" cannotbe used
# fviz_nbclust(NbClust(whole_cluster, distance = "maximum", 
#                                 min.nc = 2, max.nc = 8, method = "ward.D2", index = "all"))
...
clustered <- agnes(dist(whole_cluster, method = "maximum"), 
                   diss=TRUE, method = "ward")
...
#Clustering summaries based on non-markov approach
non_markov_clusters <-  temp_nat_adjusted %>%
  group_by(orchard) %>%
  summarise(mean_honey_ab = mean(apisAb),
            mean_wild_ab = mean(adjusted_bees),
            mean_social_rich = mean(adjusted_social)) %>% 
...
```{r}
#Non_markov clustering summary
whole_cluster %>% 
  group_by(cluster) %>%
  summarise_all(funs(mean)) %>%
  inner_join(non_markov_clusters)
