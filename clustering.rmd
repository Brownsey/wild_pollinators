...
The code in this .rmd file covers the analysis undertaken into which clusters methods perform best, given the sparse orchard data. All the Machine learning principles are applied in this section file as well as any extra exploratory data analysis. All the main findings were reported in the final dissertation report, although there are some extra exploratory data analysis and interim steps in this .rmd which are not significant enough to warrant making it to the final report. The process is repeated for both standardised and non-standardised data

### Looking at a Markov Approach
This section is going cover two step clustering based on various clustering methods for comparison.
In these scenarios each implementation will have a maximum of 8 clusters. Two after pre-bloom, 4 after and 8 after post bloom.
Of these 8 final clusters it is expected that some may be sparse with no orchards in
```{r}
load("data")
...
kmeans approach: Even running the 1001 times it is different everytime.
...
All these graphs were plotted as part of EDA by the use of lapply to the a basic plotting function. Here the relationships between some of the main extra factors (whether they are confounding or not) will be examined.
...
wild_bee_abundance_agglom <- data_2012 %>%
  ...
  geom_point(aes(colour = as_factor(node- 7))) +
  geom_smooth() +
  geom_smooth(method = "lm", colour = "red") +
  labs(x = expression("Temperature " ( degree*C)), y = "Wild Bee Abundance (log(x + 1))",
      # title = "Wild Bee Abundance Coloured by Agglomerature Nodes",
       colour = "Agglomerative\nClustering\nNode") +
  theme_bw() 
...
wild_bee_abundance_agglom_adjusted <- adjusted_data %>%
  ...
  geom_smooth() +
  geom_smooth(method = "lm", colour = "red") +
  labs(x = expression("Temperature " ( degree*C)), y = "Adjusted Wild Bee Abundance (log(x + 1))",
      # title = "Wild Bee Abundance Coloured by Agglomerature Nodes",
       colour = "Agglomerative\nClustering\nNode") +
  theme_bw() 
...
##### X2000nat
Based on previous research undertaken in the original paper it is known that the X2000nat variable has an effect on the the bee count so it also necessary to adjust for this. Wher Again the value will be adjust as though the X2000nat variable is constant. In this case the mean will be chosen as the constant value and the bee count will be adjusted for this in the same way as above. 
...
data_2012 %>%
  inner_join(orchard_node_agglom) %>%
  ggplot(aes(x = bloom.index, y = log(wildAbF + 1))) +
  geom_smooth() +
  geom_smooth(method = "lm", colour = "red") + 
  theme_bw() +  
  geom_point(aes(colour = as_factor(node - 7))) +
  labs(x = "Bloom Index (%)", y = "Adjusted Wild Bee Abundance (log(x + 1))",
      # title = "Wild Bee Abundance Coloured by Agglomerature Nodes",
       colour = "Agglomerative\nClustering\nNode") 
...
#Looking at the model ~ only X2000nat and temp significant
lm(adjusted_bees ~ temp + X2000nat + local.diversity + region + day +  bloom.index, data = logged_data) %>%
  summary
...
lm(adjusted_social ~ temp + X2000nat + local.diversity + region + day +  bloom.index, data = logged_data) %>%
  summary
...
logged_data %>%
  ...
  theme_bw() +  
  geom_point() %>%
  labs(x = "Bloom Index", y = "Bee Abundance")
```
There doesn't seem to be any obvious correlation, but just to be sure I'll split the bloom into thee categories and test these with an Anova test.
```{r}
logged_data <- logged_data %>%
...
adjusted_bee_summary$kmeans_bees %>%
  ...
  select(cluster,mean_wild_ab, adjusted, mean_social_rich, adjusted_social)
```
Potentially can see two *paths* one with a mean slightly lower than the other if you think of the structure.

### Looking at clusters
The aim of this section is to look at the clusterings from a low, medium, high point of view, so that an end user doesn't need to understand the numerical implications.
...
This section summarises all of the cluster modelled scenarios
```{r}
...
##Looking at total amount for raw data max and euclid
maximum_final %>%
   mutate(sum = rowSums(.[5:12])) %>%
  select(cluster, n , mean_wild_ab, mean_social_rich,sum) 

euclidean_final %>%
   mutate(sum = rowSums(.[5:12])) %>%
  select(cluster, n , mean_wild_ab, mean_social_rich,sum) 

#Saving the final clusters for use in the shiny app
